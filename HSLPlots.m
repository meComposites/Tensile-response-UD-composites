function HSLPlots(outSpec,inputs)
...
set(groot,...
    ...
    'defaultFigurePaperSize', [widthFig, heightFig],...
...
% Plotting data
line(outSpec.eExp*100,outSpec.XExpC/1000);
...
einfCLim=[1.2 2.2];
XinfCLim=einfCLim/100*inputs.Ef*inputs.Vf/1000;

% Create figure & axes for plot 1
figNow=figure;
axNow=gca;
colourNow=meCcolours(2,:);
...
% Plotting data
line(outSpec.einfC*100,outSpec.Fu*100,...
    'Color',colourNow);

% Create axes for plot 2
axNow=axes();
colourNow=meCcolours(3,:);
% Formatting plot 2 (XinfC vs CDF)
% axes formatting
set(axNow,'XLim',XinfCLim,'YLim',[0 100],...
    'XAxisLocation','top',...
    'YAxisLocation','right',...
    'Color','none',...
    'box','off',...
    'XColor',colourNow);
set(axNow.XAxis,'TickLabelFormat','%.1f');
set(axNow.YAxis,'TickLabels',[]);
% label formatting
set(axNow.XLabel,'String','Composite stresses (GPa)');

% Plotting data
line(outSpec.XinfC/1000,outSpec.Fu*100,...
    'Color',colourNow,'Parent',axNow);
...
XinfCLim=einfCLim/100*inputs.Ef*inputs.Vf/1000;
nBreaksLim=[0 700];
pBreaksCLim=nBreaksLim/outSpec.Vs/1000;

% Create figure & axes for plot
figNow=figure;
axNow=gca;
colourNow=meCcolours(1,:);
...
axNow=axes();
set(axNow,'XLim',XinfCLim,'YLim',pBreaksCLim,...
    'XAxisLocation','top',...
    'YAxisLocation','right',...
    'Color','none',...
    'box','off');
...
pClustersCLim=nClustersLim/outSpec.Vs;
...
for j=0:jMaxPlot
    ...
    % Plotting expected value
    line(outSpec.eExp*100,outSpec.nClustersExp(:,j+1),...
        'DisplayName','Expected value');

    % Plotting probability lines
    probColours=jet(length(inputs.pValues));
    for p=1:length(inputs.pValues)
        line(outSpec.einfC*100,outSpec.nClustersProb(:,j+1,p),...
            'Color',probColours(p,:),'LineWidth',1,...
            'DisplayName',[num2str(inputs.pValues(p)*100), 'th percentile']);
    end
    ...
    axNow=axes();
    set(axNow,'XLim',XinfCLim,'YLim',pClustersCLim(j+1,:),...
        'XAxisLocation','top',...
        'YAxisLocation','right',...
        'Color','none',...
        'box','off');
...
% Plotting expected value
line(outSpec.eExp*100,outSpec.clusterMaxLevelExp,...
    'DisplayName','Expected value');

% Plotting probability lines
probColours=jet(length(inputs.pValues));
for p=1:length(inputs.pValues)
    line(outSpec.einfC*100,outSpec.clusterMaxLevelProb(:,p),...
        'Color',probColours(p,:),'LineWidth',1,...
        'DisplayName',[num2str(inputs.pValues(p)*100), 'th percentile']);
end
...
% Create second set of axes (with composite stresses vs. density of breaks)
axNow=axes();
set(axNow,'XLim',XinfCLim,'YLim',clusterMaxLevelLim,...
    'XAxisLocation','top',...
    'YAxisLocation','right',...
    'Color','none',...
    'box','off');
